...
namespace ChatManager.Controllers
{
    public class AccountsController : Controller
    {
        ...
        public ActionResult Profil(int? id, bool? admin)
        {   
            User userToEdit = admin != null && (bool)admin ? DB.Users.Get((int)id).Clone() : OnlineUsers.GetSessionUser().Clone();
            


            if (userToEdit != null)
            {
                Session["TruePassword"] = userToEdit.Password; 
                Session["AsAdmin"] = admin != null && (bool)admin;
                Session["UserToChange"] = userToEdit;
                ...
                ViewBag.Types = SelectListUtilities<UserType>.Convert(DB.UserTypes.ToList());
                ViewBag.Genders = SelectListUtilities<Gender>.Convert(DB.Genders.ToList());
        ...
        public ActionResult Profil(User user)
        {
            ...
            {
                currentUser = OnlineUsers.GetSessionUser();
            }
            else
            {
                currentUser = (User)Session["UserToChange"];
            }
            if(user.UserTypeId == currentUser.UserTypeId)
            {
            ...
            if (ModelState.IsValid)
            {
                if (user.Password == (string)Session["UnchangedPasswordCode"])
                    user.Password = user.ConfirmPassword = (string)Session["TruePassword"];

                if (Session["AsAdmin"] != null && Session["UserToChange"] != null)
                {
                    Session["AsAdmin"] = null;
                    Session["UserToChange"] = null;
                }
        ...
        public ActionResult Login(LoginCredential loginCredential)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                Session["NotFriendsFilter"] = 0;
                Session["BlockedFilter"] = 0;
                Session["Pending"] = 0;
                Session["Request"] = 0;
                Session["Refused"] = 0;
                Session["Friend"] = 0;
